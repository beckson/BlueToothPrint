...
#define kLimitLength    512

@interface HLBLEManager ()<CBCentralManagerDelegate,CBPeripheralDelegate>

@property (strong, nonatomic,readwrite)   CBCentralManager            *centralManager;        /**< 中心管理器 */
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central
{
    ...
    NSLog(@"centralManagerDidUpdateState 更新了状态:%zd",central.state);
    switch (central.state) {
        case CBManagerStateUnknown:
            NSLog(@"centralManagerDidUpdateState 更新了状态:CBManagerStateUnknown");
            break;
        case CBManagerStateResetting:
            NSLog(@"centralManagerDidUpdateState 更新了状态:CBManagerStateResetting");
            break;
        case CBManagerStateUnsupported:
            NSLog(@"centralManagerDidUpdateState 更新了状态:CBManagerStateUnsupported");
            break;
        case CBManagerStateUnauthorized:
            NSLog(@"centralManagerDidUpdateState 更新了状态:CBManagerStateUnauthorized");
            break;
        case CBManagerStatePoweredOff:
            NSLog(@"centralManagerDidUpdateState 更新了状态:CBManagerStatePoweredOff");
            break;
        case CBManagerStatePoweredOn:
            NSLog(@"centralManagerDidUpdateState 更新了状态:CBManagerStatePoweredOn");
            break;
...
//    CBPeripheralStateDisconnecting NS_AVAILABLE(NA, 9_0),
    NSLog(@"发现了外设:%@",peripheral.name);
    switch (peripheral.state) {
        case CBPeripheralStateDisconnected:
            NSLog(@"发现了外设的状态--CBPeripheralStateDisconnected");
            break;
        case CBPeripheralStateConnecting:
            NSLog(@"发现了外设的状态--CBPeripheralStateConnecting");
            break;
        case CBPeripheralStateConnected:
            NSLog(@"发现了外设的状态--CBPeripheralStateConnected");
            break;
        case CBPeripheralStateDisconnecting:
            NSLog(@"发现了外设的状态--CBPeripheralStateDisconnecting");
            break;
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    NSLog(@"外设名字和id(连接成功时):%@====%@",peripheral.name,peripheral.identifier.UUIDString);
...
{
//    NSLog(@"外设名字和id(断开连接时):%@====%@",peripheral.name,peripheral.identifier.UUIDString);
//    NSLog(@"外设名字和id(断开连接时):%@====%@",_connectedPerpheral.name,_connectedPerpheral.identifier.UUIDString);
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(nullable NSError *)error
{
    ...
    NSLog(@"===以下是服务的特性:%@==共:%zd",service,service.characteristics.count);
    for (CBCharacteristic *character in service.characteristics) {
        NSLog(@"不常用==didDiscoverCharacteristicsForService==");
        NSLog(@"不常用==发现特性的回调代理===%@",character);
        [peripheral discoverDescriptorsForCharacteristic:character];
        [peripheral readValueForCharacteristic:character];
    }
    NSLog(@"---以上是服务的特性:%@",service);

}

- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(nullable NSError *)error
{
    NSLog(@"不常用==didUpdateNotificationStateForCharacteristic");
    NSLog(@"不常用==更新特性的回调代理===%@",characteristic);
    if (error) {
...
{
    NSLog(@"不常用==发现外设中的服务描述characteristic:%@",characteristic);
    if (error) {
        NSLog(@"不常用==发现外设中的服务描述错误");
        if (_completionBlock) {
            _completionBlock(HLOptionStageSeekdescriptors,peripheral,nil,characteristic,error);
        }
        return;
    }
    NSLog(@"不常用==发现外设中的服务描述成功");
    if (_completionBlock) {
...
{
    NSLog(@"不常用==更新了描述descriptor:%@",descriptor);
    if (error) {
...
{
    NSLog(@"不常用==已经写入数据的回调descriptor:%@",descriptor);
    if (error) {
