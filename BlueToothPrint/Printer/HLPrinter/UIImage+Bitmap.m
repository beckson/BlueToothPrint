...
- (UIImage *)imageWithscaleMaxWidth:(CGFloat)maxWidth
{
    ...
    CGFloat scale = 1.2;
    CGSize size = CGSizeMake(width, height * scale);
    if (width > maxWidth)
    {

        CGFloat maxHeight = maxWidth * height / width;

        CGSize size = CGSizeMake(maxWidth, maxHeight * scale);    //打印图片下半截打不出来
...
+ (UIImage *)barCodeImageWithInfo:(NSString *)info
{
    ...
    UIImage *image =[self createNonInterpolatedUIImageFormCIImage:outputImage withSize:360];
...
+ (UIImage *)qrCodeImageWithInfo:(NSString *)info centerImage:(UIImage *)centerImage  width:(CGFloat)width
{
    ...
    if (centerImage) {
        //icon尺寸,UIBezierPath
        CGSize logoSize = CGSizeMake(rect.size.width*0.36, rect.size.height*0.36);
