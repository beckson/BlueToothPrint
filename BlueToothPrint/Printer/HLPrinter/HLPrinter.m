...
//- (void)setLineCode:(NSString *)info {
//    Byte dataBytes [] = {0x1D,0x60,0x1D,2,0x1D,2,0x1D,0x45};
////    Byte dataBytes [] = {0x1D,'h',0x60,0x1D,'w',2,0x1D,'H',2,0x1D,'k',0x45};
...
{
    [self appendBarCodeWithInfo:info alignment:HLTextAlignmentCenter maxWidth:360];
}
...
- (void)appendQRCodeWithInfo:(NSString *)info size:(NSInteger)size alignment:(HLTextAlignment)alignment
{
    ...
    [self setQRCodeErrorCorrection:50];
 ...
 * 添加一排有文字和图片的数据
 */
- (void)appendImage:(UIImage *)image andText:(NSString *)text textAlignment:(HLTextAlignment)alignment imageMaxWidth:(CGFloat)imageWidth textFont:(HLFontSize)fontSize{
    if (!image && !text.length) {
        return ;
    }

    if (imageWidth > 0 && image && !text.length) { //只有图片
        [self appendImage:image alignment:HLTextAlignmentCenter maxWidth:imageWidth];
    }
    else if (text.length && (!image || imageWidth < 0 || imageWidth > 300)) { //只有文字
        [self appendText:text alignment:alignment fontSize:fontSize];
    }
    else {
//        UIGraphicsBeginImageContextWithOptions(CGSizeMake(300, imageWidth + 60), NO, 0);//上下30
        UIGraphicsBeginImageContext(CGSizeMake(300, imageWidth + 60));
#warning 没有设置文字大小
        if (alignment == HLTextAlignmentLeft) {    //文字在左侧
            [text drawInRect:CGRectMake(0, 30, 300 - imageWidth - 20, imageWidth) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:30],NSForegroundColorAttributeName:[UIColor blackColor]}]; //文字和图片左右间距20
            [image drawInRect:CGRectMake(300 - imageWidth , 30, imageWidth, imageWidth)];
        }
        else {
            [text drawInRect:CGRectMake(0, 30,  imageWidth + 20, imageWidth) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:30],NSForegroundColorAttributeName:[UIColor blackColor]}]; //文字和图片左右间距20
            [image drawInRect:CGRectMake(0 , 30, imageWidth, imageWidth)];
        }

        UIImage *resultImage = UIGraphicsGetImageFromCurrentImageContext();
        NSData *data = UIImagePNGRepresentation(resultImage);
        UIGraphicsEndImageContext();
        [self appendImage:resultImage alignment:HLTextAlignmentCenter maxWidth:300];
    }
...
- (void)appendFullLine {
    ...
    [self setFontSize:HLFontSizeTitleSmalle];
    // 3.添加分割线
    NSString *line = @"_______________________________";
    NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
    NSData *data = [line dataUsingEncoding:enc];
    [_printerData appendData:data];
    // 4.换行
